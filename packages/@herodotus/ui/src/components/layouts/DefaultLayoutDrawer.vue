            ...
            <template v-for="item in menu">
                ...
                >
                    ...
                    <template v-for="subItem in item.items">
                        <v-list-item
                            :key="subItem.name"
                            :to="generateChildTarget(item, subItem)"
                            :href="subItem.href"
...
import { Menu, MenuGroup, MenuItem } from '@/modules';

type target = { name: string } | undefined;

@Component
export default class DefaultLayoutDrawer extends Vue {
    @VModel({ type: Boolean, default: true }) drawer!: boolean;
    ...
    private generateChildTarget(item: MenuGroup, subItem: MenuItem): target {
        if (subItem.href) {
            return;
