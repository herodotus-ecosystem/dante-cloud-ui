  ...
  <h-authorize-layout :title="title" :overlay="overlay">
    <h-container mode="three" wider="center" :offset="4">
      <h-text-field
        ...
        :error-message="
          v.editedItem.deviceName.$errors[0] ? v.editedItem.deviceName.$errors[0].$message : ''
        "
        @blur="v.editedItem.deviceName.$validate()"
      ></h-text-field>
      <h-dictionary-select
...
export default defineComponent({
  ...
  setup() {
    const { editedItem, isEdit, title, overlay, saveOrUpdate } = useTableItem<OAuth2DeviceEntity>(
      api.oauth2Device(),
    );
