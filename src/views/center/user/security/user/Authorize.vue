                    ...
                    <v-data-table :headers="tableHeaders" :items="tableItems" :page.sync="pageNumber" :items-per-page="pageSize" :loading="tableLoading" item-key="scopeId" show-select class="elevation-1" @item-selected="selectItem($event)" @toggle-select-all="selectAllItems($event)">
                        <template v-slot:top>
                            <v-toolbar flat color="white">
                                <v-toolbar-title>系统角色列表</v-toolbar-title>
            ...
            <v-col cols="4">
                <v-card>
                    ...
                    <v-list subheader two-line shaped>
                        <v-subheader>已配置的角色</v-subheader>
                        <v-list-item-group color="primary">
                            <v-list-item v-for="item in selectedItems" :key="item.roleId">
                                <v-list-item-content>
                                    <v-list-item-title>{{item.roleCode}} - {{item.roleName}}</v-list-item-title>
...
const defaultComparator = 'roleId';
...
export default {
    ...
    methods: {
        ...
        save () {
            if (this.selectedItems && this.selectedItems.length > 0) {
                this.overlay = true;
                let userId = this.currentItem.userId;
                let roles = this.selectedItems.map(item => item.roleId);
                this.$api.upms.sysUser.assignRole({ userId: userId, roles: roles }).then(result => {
                    this.overlay = false;
