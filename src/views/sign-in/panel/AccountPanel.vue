<template>
  <q-card>
    ...
    <q-card-section>
      <q-btn v-if="hasError" align="left" outline color="negative" class="q-mb-sm full-width" :label="errorMessage"
      ...
      >
        ...
        <template #append>
          <q-icon :name="isShowPassword ? 'visibility' : 'visibility_off'" class="cursor-pointer"
                  @click="isShowPassword = !isShowPassword"/>
        </template>
      </h-text-field>

      <q-btn tabindex="3" rounded unelevated color="primary" class="full-width q-mb-md" :disable="isDisabled" label="登录"
             @click="signIn()"/>
...
<script lang="ts">
  ...
  import {RouteUtils, toast, variables} from '/@/lib/utils';
  import {useApplicationStore, useCryptoStore, useAuthenticationStore, useRouteStore} from '/@/stores';
  ...
  export default defineComponent({
    ...
    setup(props, {slots}) {
      ...
      const loc = variables.getDeployLocation();
