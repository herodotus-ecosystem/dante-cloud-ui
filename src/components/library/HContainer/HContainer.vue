...
export default defineComponent({
	...
	props: {
		 ...
		 * right-left：右边窄，左边看
		 */
		layout: { type: String as PropType<'default' | 'left-right' | 'right-left'>, default: 'default' },
		// 三列布局模式下，哪一列宽度最小
		/**
		 * 三列布局模式：
		 * default：三列相等
		 ...
		 * end：左边宽，中间默认，右边窄
		 */
		justify: { type: String as PropType<'default' | 'start' | 'center' | 'end'>, default: 'center' },
	},
	...
	setup(props) {
		...
		const isThreeColumn = computed(() => {
			return props.column === 'three';
		});

		const isTwoColumn = computed(() => {
			return props.column === 'two';
		});
